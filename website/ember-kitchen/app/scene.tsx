...
export default function BurgerScene({progress,paused,onReady,onError,onProgress}:Props){
 ...
 useEffect(()=>{
  ...
  (async()=>{
   ...
   const parts=groups.map(g=>{const geo=new T.BufferGeometry();geo.setAttribute('position',new T.Float32BufferAttribute(g.p,3));geo.setAttribute('normal',new T.Float32BufferAttribute(g.n,3));geo.setAttribute('uv',new T.Float32BufferAttribute(g.uv,2));geo.computeBoundingSphere();const mesh=new T.Mesh(geo,material);mesh.castShadow=true;mesh.receiveShadow=true;burger.add(mesh);return mesh});geometry.dispose();raw.dispose();
   // A second scroll act: a cascade of golden fries and a chilled glass of cola.
   const sides=new T.Group();scene.add(sides);
   const plateGroup=new T.Group();sides.add(plateGroup);const porcelain=new T.MeshStandardMaterial({color:0xf1e5cf,roughness:.42,metalness:.02});const plate=new T.Mesh(new T.CylinderGeometry(1.86,1.78,.09,64),porcelain);plate.scale.z=.74;plate.receiveShadow=true;plateGroup.add(plate);const plateRim=new T.Mesh(new T.TorusGeometry(1.79,.028,8,64),porcelain);plateRim.rotation.x=Math.PI/2;plateRim.scale.y=.74;plateRim.position.y=.056;plateGroup.add(plateRim);
   ...
   const fryGeometry=new RoundedBoxGeometry(.13,.92,.13,3,.025),fryMaterial=new T.MeshStandardMaterial({map:fryTexture,roughness:.72,color:0xffdd80,bumpMap:fryTexture,bumpScale:.012});const fries=new T.InstancedMesh(fryGeometry,fryMaterial,28);fries.castShadow=true;fries.receiveShadow=true;sides.add(fries);
   ...
   const reduced=matchMedia('(prefers-reduced-motion: reduce)').matches;let targetX=0,targetY=0,px=0,py=0,smoothed=progress.current,t=0,last=performance.now(),first=true,inView=true;const visibility=new IntersectionObserver(entries=>{inView=entries[0].isIntersecting});visibility.observe(host);
   function pointer(e:PointerEvent){targetX=(e.clientX/innerWidth-.5)*2;targetY=(e.clientY/innerHeight-.5)*2;}
   window.addEventListener('pointermove',pointer,{passive:true});
   const resize=()=>{if(!renderer)return;const w=host.clientWidth,h=host.clientHeight;renderer.setPixelRatio(Math.min(devicePixelRatio,w<760?1.35:1.75));renderer.setSize(w,h);camera.aspect=w/h;camera.updateProjectionMatrix()};const ro=new ResizeObserver(resize);ro.observe(host);resize();
   function render(now:number){if(disposed||!renderer)return;raf=requestAnimationFrame(render);const dt=Math.min((now-last)/1000,.04);last=now;if(!inView||document.hidden)return;const mobile=host.clientWidth<760;
    ...
    burger.position.set(mobile?-.42*meal:1.05+explode*.12-.32*meal,mobile?.10:-.12*meal,0);burger.rotation.set(.06+(reduced?0:py*.04),-.35+p*.7+(reduced?0:px*.13+Math.sin(t*.35)*.07),-.10+explode*.10);
    const fit=mobile?Math.min(1,host.clientWidth/370):Math.min(1.15,host.clientWidth/1250);burger.scale.setScalar(fit*(1-explode*(mobile?.30:.16))*(1-meal*.25));burger.position.y-=meal*.22*fit;burger.position.y+=(reduced?0:Math.sin(t*.8)*.035)*(1-meal);
    ...
    sides.visible=p>.78;sides.scale.setScalar(fit*(mobile?.78:1));sides.position.set(0,0,0);plateGroup.position.set(mobile?.25:1.24,-1.27-(1-meal)*.7,.08);plateGroup.scale.setScalar(.4+meal*.6);plateGroup.rotation.z=(1-meal)*-.12;
    glass.position.set(mobile?1.64:2.35,-1.22-(1-meal)*3.6,-.5);glass.rotation.z=-.10*(1-meal);glass.rotation.y=.15;
    for(let i=0;i<28;i++){const arrive=smooth(.79+i*.004,.90+i*.003,p),a=i*2.39996;const fx=(mobile?.50:1.55)+Math.cos(a)*(.28+(i%5)*.065),fy=-1.04+(i%6)*.065;dummy.position.set(fx+Math.sin(a)*(1-arrive)*.75,fy+(1-arrive)*4.8,.68+Math.sin(a)*.37);dummy.rotation.set(1.1+Math.sin(i)*.32+(1-arrive)*4,i*.7+(1-arrive)*3,Math.cos(i)*.7+(1-arrive)*2);dummy.scale.set(1,.75+(i%7)*.09,1);dummy.updateMatrix();fries.setMatrixAt(i,dummy.matrix);}fries.instanceMatrix.needsUpdate=true;
    ...
    camera.position.z=mobile?6.2:9;camera.position.y=mobile?.45:.45;camera.lookAt(0,mobile?0:0,0);
   ...
   cleanup=()=>{visibility.disconnect();ro.disconnect();window.removeEventListener('pointermove',pointer);env.dispose();fryTexture.dispose();const disposedGeos=new Set<Three.BufferGeometry>(),disposedMats=new Set<Three.Material>();sides.traverse(o=>{if(o instanceof T.Mesh){if(!disposedGeos.has(o.geometry)){o.geometry.dispose();disposedGeos.add(o.geometry);}for(const m of Array.isArray(o.material)?o.material:[o.material])if(!disposedMats.has(m)){m.dispose();disposedMats.add(m);}}});material.map?.dispose();material.dispose();parts.forEach(p=>p.geometry.dispose());crumbs.geometry.dispose();(crumbs.material as Three.Material).dispose();source?.geometry.dispose();};
