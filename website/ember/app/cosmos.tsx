...
export default function Cosmos({progress,onReady,onStage,onError,state}:Props){
 ...
 const mobile=()=>innerWidth<761||(innerWidth<=1024&&innerHeight<=500);const portrait=()=>mobile()&&innerHeight>innerWidth;const finePointer=matchMedia('(hover:hover) and (pointer:fine)');const reduced=matchMedia('(prefers-reduced-motion: reduce)');
 ...
 const sphere=new T.SphereGeometry(1,mobile()?64:96,mobile()?40:64);const material=(map:Three.Texture)=>new T.MeshStandardMaterial({map,roughness:.97});
 ...
 const starPositions=[],starColors=[];for(let i=0;i<(mobile()?1300:2400);i++){const a=random()*Math.PI*2,z=random()*2-1,rad=180+random()*240;starPositions.push(Math.sqrt(1-z*z)*Math.cos(a)*rad,z*rad,Math.sqrt(1-z*z)*Math.sin(a)*rad-70);const c=new T.Color().setHSL(.09+random()*.55,.15,.45+random()*.5);starColors.push(c.r,c.g,c.b)}
 ...
 const rockCount=mobile()?90:210,rocks=new T.InstancedMesh(rockGeometry,rockMaterial,rockCount),dummy=new T.Object3D();const rockData:Array<{x:number;y:number;z:number;s:number;rx:number;ry:number}>=[];
 ...
 const iceCount=mobile()?300:850,ice=new T.InstancedMesh(rockGeometry,new T.MeshStandardMaterial({color:'#a69476',roughness:.85}),iceCount);
 ...
 // Portrait compositions look directly at each world. The reveal pulls back far enough to fit the rings.
 const phoneStops=[
 {p:0,c:[4.4,2,19],t:[4.4,.8,0]},
 {p:.17,c:[14,5,15],t:[4.4,.8,0]},
 {p:.26,c:[2,3,-5],t:[3.8,1,-28]},
 {p:.34,c:[-4,3,-13],t:[3.8,1,-28]},
 {p:.44,c:[0,1,-34],t:[0,0,-61]},
 {p:.52,c:[-1,2,-53],t:[0,1,-89]},
 {p:.60,c:[0,12,-60],t:[0,2,-115]},
 {p:.69,c:[2,15,-69],t:[0,2,-115]},
 {p:.79,c:[17,10,-83],t:[0,2,-115]},
 {p:.87,c:[17,3,-108],t:[0,2,-115]},
 {p:.94,c:[13,12,-145],t:[0,2,-115]},
 {p:1,c:[0,20,-170],t:[0,2,-115]},
 ];
 const position=new T.Vector3(),look=new T.Vector3(),nextPosition=new T.Vector3(),nextLook=new T.Vector3(),pointer=new T.Vector2();
 const mouse=(e:PointerEvent)=>{if(e.pointerType==='mouse'&&finePointer.matches)pointer.set(e.clientX/innerWidth-.5,e.clientY/innerHeight-.5)};
 addEventListener('pointermove',mouse,{passive:true});cleanup.push(()=>removeEventListener('pointermove',mouse));
 // Observe the actual scene region, so the phone's shorter canvas keeps the correct aspect ratio.
 const resize=()=>{const width=Math.max(1,host.clientWidth),height=Math.max(1,host.clientHeight);camera.aspect=width/height;camera.fov=mobile()?46:43;camera.clearViewOffset();if(mobile()&&!portrait())camera.setViewOffset(width,height,-width*.23,0,width,height);camera.updateProjectionMatrix();r.setPixelRatio(Math.min(devicePixelRatio,mobile()?1.3:1.6));r.setSize(width,height,false)};
 const observer=new ResizeObserver(resize);observer.observe(host);addEventListener('resize',resize);cleanup.push(()=>{observer.disconnect();removeEventListener('resize',resize)});resize();
 let smoothP=Number.isFinite(progress.current)?progress.current:0,previous=performance.now(),elapsed=0,first=true;
 const animate=(now:number)=>{if(disposed)return;const dt=Math.min((now-previous)/1000,.05);previous=now;if(!document.hidden){elapsed+=reduced.matches?0:dt;smoothP=reduced.matches?progress.current:T.MathUtils.damp(smoothP,progress.current,5,dt);if(Math.abs(smoothP-progress.current)<.00015)smoothP=progress.current;const flight=mobile()?phoneStops:stops;let k=0;while(k<flight.length-2&&smoothP>flight[k+1].p)k++;const a=flight[k],b=flight[k+1];let q=T.MathUtils.clamp((smoothP-a.p)/(b.p-a.p),0,1);q=q*q*(3-2*q);position.set(...a.c as [number,number,number]).lerp(nextPosition.set(...b.c as [number,number,number]),q);look.set(...a.t as [number,number,number]).lerp(nextLook.set(...b.t as [number,number,number]),q);if(!mobile()&&finePointer.matches&&!reduced.matches){position.x+=pointer.x*.32;position.y-=pointer.y*.2}camera.position.copy(position);camera.lookAt(look);
